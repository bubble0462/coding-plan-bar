...
function main() {
  ...
  const notes = [
    ...
    "## 视觉与交互精修",
    "",
    "- 分离服务可用状态与额度风险：可用账号不再因单个额度接近上限而整卡显示为故障。",
    "- 高额度风险改为在具体额度行显示“需要关注”或“接近上限”，真正的查询失败和凭据问题仍保留整卡警示。",
    "- 风险摘要现在聚焦问题卡片本身，不再把键盘焦点误放到排序手柄。",
    "- 移除整窗刷新扫光、按钮扫光、主按钮呼吸、导航状态点 pulse 和面板悬浮等持续装饰动效。",
    "",
    "## 设置与导入",
    "",
    "- 诊断中心只保留一个刷新入口，底部状态栏不再重复显示同一操作。",
    "- “最新文件”明确从 Downloads 查找最新的 sub2api JSON 文件。",
    "- 导入预览合并重复的独立额度说明，并弱化值为 0 的更新与跳过统计。",
    "- 增加风险焦点、状态分层、诊断单一操作和导入预览层级的回归断言。",
    "",
