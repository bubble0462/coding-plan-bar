# Changelog

## [0.8.7] - 2026-08-28

### Added

- toast 轻提示：保存配置、导入账号、测试连通的结果会从右上角滑入玻璃小条（成功绿点 / 失败红点），与底部状态行互补。
- 气泡提示：设置页顶栏、侧栏与编辑器头部的按钮，弹窗的刷新 / 设置 / 退出按钮，悬停显示说明气泡（`data-tip` 即用，颜色走主题变量）。
- 分段控件滑动指示条：备份页代理 / 密度 / 主题与通知页开关的分段控件，激活态改为滑动的药丸指示条。
- 按钮加载态：测试连通、获取模型、发送、测试连接与模型、检查更新在等待时按钮内嵌转圈。
- 弹窗 count-up：数据刷新时变化的百分比数字从旧值缓动滚动到新值（首次渲染保持静态，尊重系统「减少动态效果」设置）；开关按住时小球回弹（iOS 手感）。

### Changed

- 深色模式整体重设计：弹窗与设置页改为「背景最暗 → 面板 → 卡片更亮」的亮度阶梯，卡片真正浮起；状态色改为透明度叠色，边框、玻璃、终端、主按钮等全部收敛为主题 token。设置页 50+ 处硬编码浅色（顶栏玻璃、模态、徽章、导入向导、用量页等）全部主题化，深色下不再出现浅色残留或反白的回复区。
- 弹窗详情页 24h 用量图表从写死白底改为主题化：深色下为深底亮蓝线。
- 深色弹窗 / 设置页截图门禁接入发布流程（亮度阶梯、无残留白色断言），新增 toast、分段滑动、按钮加载、count-up 与滚动保持断言。

### Fixed

- 设置页任何重渲染（切换分段、开关、编辑字段）不再丢失滚动位置：此前备份页点击「舒适/紧凑」「浅色/深色」等分段会立刻跳到页首；视图或选中供应商切换时仍回到顶部。

## [0.8.6] - 2026-08-27

### Added

- 全供应商统一的「模型与对话测试」卡片：Codex、Antigravity 与全部 Coding Plan（智谱 GLM、Z.AI、Kimi、Qwen、MiniMax、ZenMux 等）都支持先点击「获取模型」拉取实时模型清单，再选择模型发送一条真实消息做对话测试（流式显示回复）。Antigravity 走 cloudcode-pa 网关（Gemini 模型），Coding Plan 走 OpenAI 兼容接口，各家认证方式自动适配。

### Changed

- Codex 的模型列表从「本地缓存 + 硬编码兜底」升级为实时请求 ChatGPT 后端（失败时自动回退本地缓存并在结果里注明来源）。
- 下拉菜单按组件库设计重绘：玻璃拟态浮层（backdrop-blur）、打开时品牌色聚焦环、箭头旋转、悬停色洗底、选中项「✓」勾选、上浮缩放入场，颜色全部走主题变量，浅/深主题自动适配；打开后焦点自动落进选项，↑↓ 环绕导航并把当前项滚动到可视区。
- Coding Plan 供应商的旧「连接测试」卡片并入统一卡片（获取模型即连通测试）；余额类供应商保留原端点探测；Claude/Grok 订阅显示明确的「暂不支持对话测试」说明（可用「测试连通」验证凭据与额度）。
- 「测试连通」从 Codex 专属扩展到全部官方订阅（Codex、Claude、Grok、Antigravity），执行一次只读额度查询并显示档位利用率与诊断信息。
- 「添加供应商」的自定义模板现在明确列出可接入的供应商清单：Coding Plan（智谱 GLM、Z.AI、Kimi、Qwen 百炼、MiniMax、ZenMux）、余额查询（DeepSeek、Moonshot、OpenRouter、硅基流动及兼容 `/v1/usage` 的中转站）与手动额度，均按请求地址自动识别。

## [0.8.5] - 2026-08-25

### Fixed

- 供应商额度接近上限、已经用尽或余额为零时，不再计入弹窗的「账户需要处理」、设置页「需处理」筛选或诊断中心；额度进度条、风险颜色和「额度用尽」状态仍正常保留。
- 可识别的额度耗尽响应（例如 `insufficient_quota`、`quota exhausted`、余额不足）现在单独归类为额度状态，不再误报为账号配置或服务故障。
- 额度耗尽型 HTTP 402/403/429 不再触发「服务异常」系统通知；真正的认证失败、token 过期、配置缺失和查询错误仍会提醒并进入诊断中心。

### Changed

- 诊断中心文案与筛选逻辑统一为只展示用户能够处理的账号配置、凭证和查询故障。
- 增加额度 90%/100%、零余额、额度耗尽响应和真实登录过期的回归测试，防止额度风险再次混入账户健康诊断。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。

## [0.8.3] - 2026-08-23

### Fixed

- 弹窗改为「可见期间绝不调整窗口尺寸」策略，彻底杜绝供应商选择器行被遮挡：透明窗口在可见状态下的任何尺寸变化都可能在部分机器上留下绘制残影（内容整体上移、盖住选择器行）。现在高度只在弹窗隐藏时按渲染端实测值调整，可见时如果内容变高，由供应商列表内部滚动消化——选择器行与页头固定在顶部，永远不会被内容覆盖。切换供应商或刷新期间窗口高度保持稳定，收起再展开后按最新内容重新适配。
- 刷新期间每个快照都会触发的弹窗定位调用增加坐标去重：位置没有真正变化时不再发起原生窗口移动（此前一次刷新约 9 次无效 setPosition）。

## [0.8.2] - 2026-08-23

### Fixed

- 彻底修复弹窗刷新后顶部供应商选择器行被遮挡的问题。三重加固：
  1. 可见弹窗的高度收缩改为尾随防抖（约 220ms 静默后才生效，期间任何新的高度上报都会替换它）——瞬时矮测量后紧跟正确高度的「先缩后涨」序列在物理上无法再到达窗口，而这正是透明窗口绘制残影的触发条件；增高仍然立即生效，内容不会被裁切。
  2. 弹窗尺寸与位置改为一次原子 `setBounds` 调用（此前 setSize 与 setPosition 是两次独立原生调用，底边锚定的弹窗在两步之间会合成一帧错误位置的画面并被 DWM 缓存为残影）。
  3. 详情视图在同一布局形态内不再回传比上次明显更矮的瞬时高度（图表在重建后的重排瞬间会短暂塌陷）；真正的收缩都伴随布局形态变化，不受影响。

## [0.8.1] - 2026-08-23

### Fixed

- 修复 Antigravity 账号在设置页被误标为「过期 / 将过期」的问题：凭证文件里的 `expired` 只是导出时刻访问令牌的寿命，应用实际用 refresh_token 自动续期（查询始终成功）。现在账号卡片显示「续期：自动」，过期徽标、健康页与需要关注列表都不再依据该静态时间判定 Antigravity 凭证状态；只有 refresh token 真正失效（刷新被 Google 拒绝）时才显示过期。
- 修复供应商详情页刷新时顶部选择器行被内容遮挡的问题（v0.7.2 同类竞态在详情页的复现）：供应商数超过滚动阈值时，详情页首帧会按「前 N 张卡片」的滚动公式误算出远小于实际的高度，33ms 后又修正回去，可见窗口上先缩后涨触发绘制错位。现在详情页一律按整页实测高度上报，且主进程对可见窗口 250ms 内的大幅回缩做防抖拦截。
- Antigravity 套餐显示改为 paidTier 优先（与主流账号管理工具一致）：Pro 账号的 currentTier 仍是 Code Assist 的 free-tier，但 paidTier 是 g1-pro-tier，此前误显示为 Free。

## [0.8.0] - 2026-08-23

### Added

- 新增 Antigravity（Google）供应商：在设置的账号页导入 `antigravity-*.json` 凭证文件即可显示额度，支持同一邮箱重复导入更新与多账号并存；也可留空自动读取本机 Antigravity 登录（只读）。额度取自 Google Cloud Code Assist 网关的 `fetchAvailableModels`，只展示 Gemini 系模型的池化窗口：同一重置时间聚合为一档，剩余有效期超过 5.5 小时判定为周额度，否则为 5 小时额度（免费账号通常只有单窗口）。套餐名按 currentTier 显示（Free / Google AI Pro / Google AI Ultra）。
- 凭证含 `refresh_token` 时自动向 Google OAuth 续期访问令牌（进程内缓存，磁盘只存加密的凭证 JSON），token 失效时给出明确的重新导入提示；Claude/GPT 系模型的共享池按需求不展示。
- 账号 JSON 导入、下载目录自动识别、导入历史、供应商筛选与模板选择器全面支持 antigravity 格式（`type: "antigravity"` 或 `antigravity-*.json` 文件名，且优先于 CPA 的「邮箱+套餐」文件名规则判定）。
- 自适应刷新的本机 agent 进程检测加入 Antigravity（IDE 运行时把刷新间隔压到 90 秒）。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。

## [0.7.2] - 2026-08-22

### Fixed

- 修复点击「刷新」或后台数据刷新时，弹窗顶部供应商选择器行被头部区域遮挡的问题。弹窗可见期间不再执行估算高度的连续窗口缩放，只采用渲染进程实测上报的高度（v0.6.0 同类修复的推广，覆盖数据刷新路径；自适应刷新使其更易复现）。
- 修复周额度重置的误判：启动基线来自本地缓存（窗口在应用关闭期间早已滚动）或两次刷新间重置时间戳轻微抖动时，不再触发「周额度已重置」的通知与庆祝。现在只有"上一次观测时窗口尚未结束、本次观测时已结束"的真实滚动才会触发。
- 「周额度已重置」提示条改为显示在弹窗底部（退出按钮上方），不再位于选择器附近。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。

## [0.7.1] - 2026-08-22

### Added

- 供应商编辑器新增「连接测试」卡片（Coding Plan / 余额 / 手动类型可用）：一键用当前请求地址与 API Key 查询端点连通性与可用模型列表，显示 HTTP 状态、延迟、命中的接口地址和模型清单（只读，不消耗对话额度）。自动尝试 OpenAI 兼容的 `/models`、`/v1/models`，并为百炼/智谱等专属路径内置候选；未保存的编辑值与已保存密钥均可用于测试。
- 添加供应商分栏预览明确标注「API Key 添加后在右侧编辑器填写」，避免误以为弹窗内缺少密钥输入。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。连接测试按只读 GET 请求探测模型列表；个别供应商不提供模型列表接口时会显示「服务器可达」与原因，不影响额度查询。

## [0.7.0] - 2026-08-20

### Added

- 系统通知：额度档位越过 80%/95% 提醒线、周额度等窗口重置、供应商查询失败时发送 Windows 通知，点击可打开额度面板；每个额度窗口只提醒一次，重置后自动恢复。设置页新增「通知与提醒」页，可开关总开关、重置提醒与异常提醒。
- Qwen（阿里百炼）Coding Plan 供应商：5 小时 / 周 / 月额度与重置倒计时，DashScope API Key（支持 `DASHSCOPE_API_KEY` 环境变量）。中国站优先、国际站自动回退；个别账户要求百炼网页会话时会明确提示，不影响其它供应商。
- Kimi 凭据自动复用：未配置 API Key 时只读复用本机 Kimi Code CLI 登录凭据（`~/.kimi-code/credentials`），不写入、不刷新。
- 周额度重置庆祝：检测到周额度窗口重置时，弹窗播放一次性彩带动画并显示「周额度已重置」提示（尊重系统减弱动态效果设置）。
- 「添加供应商」改为左右分栏选择器：左侧按 官方订阅 / Coding Plan / API 余额 / 自定义 分组，支持 ↑↓ 键盘导航；右侧实时预览所选模板的说明、需要什么凭据、环境变量与控制台地址。

### Changed

- 自适应刷新：额度窗口临近重置（5 分钟内）时自动加密轮询（30 秒），检测到本机正在运行 Codex/Claude/ZCode/Kimi/Qwen 等 agent 时缩短到 90 秒，其余时间保持配置间隔；进程检测每 5 分钟最多一次，失败时静默退回纯定时。
- 发布门禁新增弹窗总览、DeepSeek 详情、模板选择器与通知页截图检查。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。Qwen 额度接口为百炼控制台非公开接口，未来可能随服务端调整需要更新适配；Kimi CLI 凭据只在本机读取。Windows「专注助手」开启时系统会暂存通知。

## [0.6.3] - 2026-08-19

### Fixed

- 修复 0.6.2 中总览行多档位短进度条没有颜色填充（只显示空白轨道）的问题。短条现在与详情页进度条使用完全相同的配色与渲染方式，并新增截图门禁断言填充实际可见。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。

## [0.6.2] - 2026-08-19

### Fixed

- 修复 GLM「今日调用 / Token」实际统计的是近 24 小时（含昨日用量）的问题。现在额外请求自然日（今天 0 点至现在）窗口，「今日」严格按自然日统计；折线图仍展示近 24 小时。今日请求失败时回退为原口径。

### Changed

- 弹窗总览行同时展示多个额度档位：如 GLM 的 5 小时与周额度分别以短标签 + 细进度条并排显示各自剩余百分比（悬停可看已用明细）；只有单一档位（如 Grok 周期限额）或余额型供应商保持原有展示不变。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。GLM 用量为智谱服务端口径的统计数据，不代表订阅账单。

## [0.6.1] - 2026-08-18

### Fixed

- DeepSeek 峰谷提示按当前官方规则更新：北京时间 09:00–12:00、14:00–18:00 为高峰，其余为空闲 5 折时段；提示仍不参与平台返回费用的重复计算。
- DeepSeek 平台用量兼容按模型嵌套的新响应结构和显式 `COST_CNY` 费用字段，避免新旧字段同时存在时重复累计费用。
- DeepSeek 平台请求补充控制台兼容请求头，并区分 Token 失效、请求限流、平台风控与非 JSON 响应；`429` 不再立即重试。
- DeepSeek 平台 Token 纳入额度缓存指纹，更换平台账号后不会短暂显示上一个账号的用量。
- 修复额度弹窗总览在 4–6 个供应商时过早进入固定高度滚动的问题，现与“最多直接显示 6 个”规则一致。
- 修复通用余额总览把非 CNY 货币统一显示为美元符号的问题。

### Improved

- 弹窗总览供应商行支持 Tab 聚焦，并可使用 Enter 或空格打开详情；补充清晰的键盘焦点样式。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。DeepSeek 平台用量来自非公开控制台接口，未来仍可能随服务端调整而需要更新适配；官方余额 API 不受该功能影响。

## [0.6.0] - 2026-08-18

### Fixed

- 修复 0.5.0 中进入单个供应商详情后顶部供应商选择器被遮挡消失的问题（详情内容超高时可整体滚动，选择器与底部按钮始终可见）。
- 修复 0.5.0 中从供应商详情切回「全部」时，MCP 额度等详情区块残留显示的问题。

### Added

- 弹窗改为「总览 / 详情」两级视图：总览以紧凑行展示每家供应商的剩余额度与风险状态（细进度条、风险变色、服务状态红点），点击任意行进入该供应商详情，点「全部」返回。
- DeepSeek 供应商详情新增平台用量面板：本月费用（¥）、本月 Token、缓存命中/未命中/输出明细、当月逐日费用折线与峰谷时段徽章（北京时间 00:30–08:30 谷时 5 折，仅为提示、不影响费用计算）。需在设置中填写 DeepSeek 平台 Token（platform.deepseek.com 控制台获取，可选）；未配置时显示引导提示，Token 失效时提示更新，余额查询均不受影响。平台 Token 通过 Windows DPAPI 加密存储。
- GLM 详情视图底部新增数据来源说明。

### Changed

- 总览行保留拖拽排序、待处理汇总与变更闪烁反馈；高度估算与窗口尺寸随之调整（总览最多直接展示 6 行，超出可滚动）。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。DeepSeek 平台用量来自 DeepSeek 开放平台控制台接口，非公开 API，Token 可能过期；不配置即保持原有余额查询体验。已有供应商、账号与 API Key 配置不受影响。

## [0.5.0] - 2026-08-18

### Added

- 弹窗顶部新增供应商选择器：可在「全部」（原多卡片视图，保留拖拽排序与待处理汇总）与单个供应商详情之间切换。选择会持久保存，悬停重开与重启后保持；默认选中 GLM 供应商（未配置 GLM 时为「全部」）。
- GLM（智谱/Z.ai）详情视图新增今日用量面板：今日调用次数、Token 消耗、近 24 小时调用折线图（白底蓝线）与 MCP 月度额度进度条，数据来自智谱服务端 model-usage 接口。接口失败时自动降级为仅卡片视图。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。今日用量与 24h 折线图为服务端口径的估算数据，不代表订阅账单。已有供应商、账号和 API Key 配置不受影响。

## [0.4.23] - 2026-08-09

### Removed

- 移除 ZCode 用量统计。「Agent 用量」恢复为 Codex 与 Claude Code 两档。原因是 ZCode 本地日志（cli/agents 的子代理 transcript + cli/rollout 的主会话 model-io）只覆盖部分用量，与服务端统计相差较大，无法准确反映实际消耗。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。Codex 与 Claude Code 的用量统计不受影响。

## [0.4.22] - 2026-08-09

### Added

- 「Agent 用量」新增 ZCode 统计：可在 Codex、Claude Code 与 ZCode 之间切换，统计本机 `~/.zcode/cli/agents` 的今天、最近 7 天和最近 30 天的请求、Token、会话和模型费用。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。ZCode 用量只读取本地 JSONL 会话日志，不上传任何内容；费用按公开 API 单价估算，不代表订阅账单。

## [0.4.21] - 2026-08-09

### Fixed

- 修复默认设置窗口宽度下「Agent 用量」标题栏的「重新统计」按钮被挤出可视区域的问题。标题描述现在会自适应省略，操作区固定宽度不被压缩。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。

## [0.4.20] - 2026-08-09

### Added

- 「Agent 用量」新增 Claude Code 统计：可在 Codex 与 Claude Code 之间切换，统计本机 `~/.claude/projects` 的今天、最近 7 天和最近 30 天的请求、Token、会话和模型费用。缓存 Token 按 Anthropic 计费方式单独统计。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。Claude Code 用量只读取本地 JSONL 会话日志，不上传任何内容；费用按公开 API 单价估算，不代表订阅账单。

## [0.4.19] - 2026-08-03

### Removed

- 移除 OpenCode Agent 用量统计功能。「Agent 用量」页面现在只统计本机 Codex Agent 用量，不再调用 OpenCode CLI 或读取 opencode.db。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。已配置的供应商、账号和 API Key 不受影响；agent-usage 缓存格式去掉 opencode 源，旧缓存会在下次刷新后自动失效重建。

## [0.4.18] - 2026-08-03

### Added

- 新增深色模式：设置页「外观主题」可在浅色 / 深色之间切换，悬停弹窗和设置窗口同步应用。
- 选择会立即生效并随配置保存，下次启动沿用上次选择；默认仍为浅色，不影响现有用户。

### Changed

- 弹窗和设置窗口的配色全面变量化（约 400 处硬编码颜色迁移到语义变量），为深色主题提供一致的色板，浅色外观保持不变。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。本次为纯视觉特性，不改变额度查询、凭据或配置格式。

## [0.4.17] - 2026-08-03

### Changed

- 本地 Agent 会话用量扫描（Codex `~/.codex/sessions`、Claude `~/.claude/projects` 等 JSONL 目录）的最小刷新间隔由 5 分钟放宽到 15 分钟，减少空闲期的磁盘扫描与 CPU 占用。额度查询频率不变，仍按设置的刷新间隔进行。

### Performance

- 启用 Grok 供应商时，每次刷新不再重复通过 `where.exe` / `which` 查找 grok 可执行文件路径，首次解析后整个生命周期复用，未安装时不缓存以便后续安装能被检测到。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。本次为后台资源占用优化，不改变任何额度查询、凭据或配置格式。

## [0.4.16] - 2026-07-26

### Added

- 支持导入 CPA 导出的 Claude OAuth 账号 JSON，例如 `claude-user@example.com.json`，导入后可独立查看 Claude 官方订阅额度。
- 「最新文件」、选择文件、拖放和粘贴 JSON 均支持 Claude 账号格式；同一邮箱再次导入会更新账号，不会与同邮箱 Codex 账号合并。

### Security

- Claude 导入仅保存额度查询所需的 access token，并使用 Windows DPAPI 加密；refresh token 和 ID token 不保存、不展示。
- 已过期的 Claude access token 会直接标记为过期，不再发送额度查询请求。

### Notes

升级前请从系统托盘完全退出旧版本。拿到新的 Claude 账号 JSON 后，可放入 Downloads 点击「最新文件」，或通过「导入」选择、拖放或粘贴。

## [0.4.15] - 2026-07-25

### Changed

- 额度达到 90% 才进入「诊断中心」、左侧「需处理」筛选和弹窗待处理汇总。
- 70% 起仍保留黄色额度条和「关注」提示，但不再作为需要处理的问题。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。token 即将过期、查询失败和余额不足仍会正常进入诊断中心。

## [0.4.14] - 2026-07-25

### Changed

- OpenCode「今天 / 7 天 / 30 天」统一从本机 `opencode.db` 按模型汇总 Token，并按公开 API 单价估算费用；provider 有非零记录时仍优先用记录值。
- 修复今天/7 天卡片因缺少模型拆分而显示「未定价」或仅显示极少记录费用的问题。

### Notes

升级前请从系统托盘完全退出旧版本。打开 Agent 用量后点「重新统计」刷新。估算费用不等于订阅账单。

## [0.4.13] - 2026-07-24

### Fixed

- 「最新文件」可识别新版 CPA 导出文件名，例如 `codex-name@gmail.com-plus.json`；不再要求文件名必须包含 `cpa`。
- Downloads 扫描增加近期 JSON 内容识别兜底，避免 CPA 导出改名后找不到。

### Notes

升级前请从系统托盘完全退出旧版本。把 CPA 导出 JSON 放到 Downloads 后点「最新文件」即可。

## [0.4.12] - 2026-07-24

### Added

- OpenCode 模型费用：provider 记录为 $0 或缺失时，按公开标准 API 单价估算（与 Codex 估算口径一致，不等同订阅账单）。
- 定价表补充 Grok / Gemini 常见型号。

### Notes

升级前请从系统托盘完全退出旧版本。打开 Agent 用量后可点「重新统计」刷新。

## [0.4.11] - 2026-07-24

### Fixed

- 应用数据目录移出安装目录：默认改为 `D:\Apps\Coding Plan Bar Data`，避免 NSIS 升级/重装清空账号配置。
- 启动时自动迁移旧路径 `D:\Apps\Coding Plan Bar\Data`、`D:\Coding Plan Bar\Data` 与 `%APPDATA%` 中的配置（不覆盖已有文件）。

### Notes

升级前请从系统托盘完全退出旧版本。若账号仍缺失，可从备份恢复 `config.json` 到新数据目录。

## [0.4.10] - 2026-07-24

### Changed

- 精简 OpenCode Agent 用量页说明文案，去掉多余的「今天」时间口径提示。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。

## [0.4.9] - 2026-07-24

### Changed

- OpenCode「今天」改为读取本机 `opencode.db`，按本地 0 点切割；近 7/30 天仍用 `opencode stats --pure`。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。首次打开 Agent 用量可点「重新统计」刷新缓存。

## [0.4.8] - 2026-07-23

### Changed

- Agent 用量页面逻辑拆到 `settings/agent-usage-view.js`，减小 `settings.js` 体积。

### Fixed

- 截图脚本 `chat:list-codex-models` mock 改为返回完整 `{slug,label}` 列表，避免探测卡片反复加载并在 capture 时刷屏报错。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。

## [0.4.7] - 2026-07-23

### Changed

- 默认应用数据目录改为 `D:\Apps\Coding Plan Bar\Data`（与安装目录同树）。
- 升级时自动从旧路径 `D:\Coding Plan Bar\Data` 与 `%APPDATA%\Coding Plan Bar` 迁移配置/缓存（不覆盖已有文件）。

### Notes

升级前请从系统托盘完全退出旧版本。若需自定义目录，可设置环境变量 `CODING_PLAN_BAR_DATA_DIR`。

## [0.4.6] - 2026-07-23

### Fixed

- OpenCode Agent 用量「缓存占输入」误按 `cache / input` 计算并被封顶为 100%。现按 Anthropic / cc-switch 口径改为 **缓存命中率** `cache / (input + cache)`；Codex 仍使用 `cache / input`。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。

## [0.4.5] - 2026-07-23

### Fixed

- 修复迁移到 D 盘后启动即崩溃：`normalizeUnavailableSecretFields` 未导出导致读配置时直接抛错。
- 单条 DPAPI 凭据无法解密时改为标记该账号需重新授权，不再拖垮整个应用启动；写回配置时保留原密文，不误删其它账号。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。若个别账号显示需重新填写 API Key / 重新导入 token，仅补该账号即可。

## [0.4.4] - 2026-07-23

### Added

- 应用启动后立即在后台预热 Codex 与 OpenCode Agent 用量；设置页优先显示最近一次可用汇总，刷新在后台进行。
- 应用自己的配置、配额缓存、Agent 用量汇总和 Electron 会话缓存默认保存到 `D:\Coding Plan Bar\Data`，并支持 `CODING_PLAN_BAR_DATA_DIR` 覆盖。

### Changed

- Agent 用量缓存只保存聚合后的统计快照（最大 512 KB），不复制原始会话或聊天内容。
- 每日清理过期临时文件，应用日志最多保留 14 天或 10 MB，并清理 Electron HTTP 缓存。
- 首次升级会安全迁移旧 `%APPDATA%\Coding Plan Bar` 应用数据；仅复制成功后才删除旧目录，D 盘不可用时自动回退。

### Fixed

- Codex 或 OpenCode 单个来源的本次统计失败时保留上次成功结果，不再让另一个来源或整个 Agent 用量页失效。
- 已缓存的 Agent 用量在首次打开设置页时立即显示，并以后台刷新状态提示代替整页加载等待。


## [0.4.3] - 2026-07-23

### Added

- 设置页「Agent 用量」新增 OpenCode 本机统计：读取今天、最近 7 天和最近 30 天的会话、请求、Token、provider 记录费用与模型明细。
- Agent 用量页顶部加入 Codex / OpenCode 图标切换；首次进入默认显示 OpenCode。

### Fixed

- Codex 会话扫描异常时不再阻断 OpenCode 统计；两个来源独立降级并显示各自错误信息。
- OpenCode CLI 不存在、超时或返回异常时提供明确提示，不影响 Codex 用量查看。

### Notes

- OpenCode 数据仅来自本机 `opencode stats --pure`，不会读取或上传聊天内容。
- provider 未记录价格时仍显示 Token 与请求数，费用可能为 `$0.00`。

## [0.4.2] - 2026-07-21

### Fixed

- 修复 Grok 额度解析中 `firstNumber` 重复定义：`null` 不再被误当成 `0`，月度积分与按量付费字段在缺失时正确显示为空。
- 修复设置页开关（启用/停用供应商、自动更新等）点击时因未定义的 `pulseToggle` 中断事件处理的问题。
- 修复额度刷新 `finally` 中不安全的 `return`，避免吞掉刷新任务异常。

### Changed

- `npm run check` 改为自动扫描 `src/` 与 `scripts/` 下全部 `.js` 做语法检查，并接入 ESLint。
- 主进程 timer（刷新 / 隐藏弹窗）集中到 `AppTimers`，退出时统一清理。
- HTTP 请求增加 `fetchWithTimeout` 封装；Grok Web Billing 默认走该超时路径。
- 设置页纯格式化函数拆到 `settings/helpers.js`，降低 `settings.js` 体积。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。

## [0.4.1] - 2026-07-20

### Changed

- 代理支持与连通性修复（系统 / 直连 / 手动代理）。
- 额度查询、测试连通、对话探测改走 Electron Chromium 网络栈。
- 修复「测试连通」脱敏 token 与利用率显示错误等问题。
## [0.8.4] - 2026-08-23

### Fixed

- 修复 Windows 透明无边框弹窗在刷新和切换供应商时逐次增高、底部空白不断累积的问题。根因是托盘底部对齐逻辑混用 `getSize()`、`getBounds()` 与 `setPosition()`；部分 Windows 缩放环境下内容尺寸与外框尺寸相差 1px，每次重新定位都会让窗口继续增高。现在统一使用外框坐标，并通过一次原子 `setBounds()` 同时保持位置和宽高。
- 修复 GLM 详情较高时顶部供应商选择器被压缩、按钮下半截被裁切的问题。页头、选择器和底栏不再参与 Flex 收缩，GLM 详情内容保持自然高度。

### Changed

- 恢复供应商详情动态高度：从「全部」切换到 GLM 等详情时只进行一次自然高度适配，切回总览时恢复紧凑高度；同一详情内刷新数据不再重复修改原生窗口尺寸。只有自然高度超过屏幕工作区时才在详情区域内部滚动。
- 详情高度改为直接测量不可收缩的内容块，不再使用会受当前窗口高度反向影响的 `scrollHeight`，避免测量值随窗口增长而继续增长。
- 增加弹窗几何回归门禁，覆盖 GLM 详情展开、刷新后高度稳定、选择器完整可见、详情不越过选择器，以及禁止重新引入危险的尺寸 API 组合。

### Notes

升级前请从系统托盘完全退出旧版本，再运行新安装包。
