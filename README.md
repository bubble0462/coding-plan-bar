...
- **导入**：打开统一导入卡片，可拖入 JSON 文件、通过文件选择器选择文件，或直接粘贴 JSON 内容；预览确认后立即加密保存账户。
- **添加**：打开供应商模板选择器，用于新增和配置供应商。这是三个按钮中唯一用于添加供应商的按钮，不负责导入账户 JSON。

“最新”和“导入”服务于账户 JSON 导入；导入完成后，账户会出现在供应商列表中，并可单独启用、停用、排序和刷新额度。确认导入会直接写入本机配置，无需再次点击保存。导入过程只显示必要的账户摘要，不会在预览中展示 OAuth token 或 API Key 原文。
