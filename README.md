...
- 统一「模型与对话测试」：Codex、Antigravity 与全部 Coding Plan 供应商均可先实时获取模型清单，再选模型发送一条真实消息验证（流式显示回复）；「测试连通」覆盖全部官方订阅（Codex、Claude、Grok、Antigravity）。
- 深色模式整体重设计：亮度阶梯分层卡片、主题化图表与状态色，浅/深两套主题统一收敛到设计令牌。
- 交互增强：操作结果 toast 轻提示、按钮悬停气泡与加载转圈、分段控件滑动指示条、刷新时百分比数字滚动。
