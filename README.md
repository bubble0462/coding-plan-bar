...
设置页左侧“账号与供应商”标题旁有四个快捷按钮。它们的用途不同：

- **最新**：自动查找 Downloads 文件夹中最近生成的 sub2api 格式 JSON 文件，生成预览后导入其中的账户。
- **粘贴**：粘贴 sub2api 导出的 JSON 内容，预览并导入其中的账户，不需要先保存为文件。
- **导入**：通过文件选择器选择 sub2api 格式的 JSON 文件，预览确认后导入账户。
- **添加**：打开供应商模板选择器，用于新增和配置供应商。这是四个按钮中唯一用于添加供应商的按钮，不负责导入 sub2api 账户文件。

“最新”“粘贴”“导入”服务于账户 JSON 导入；导入完成后，账户会出现在供应商列表中，并可单独启用、停用、排序和刷新额度。导入过程只显示必要的账户摘要，不会在预览中展示 OAuth token 或 API Key 原文。
